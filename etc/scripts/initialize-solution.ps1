...
$jobs += Start-Job -Name "DbMigrator" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/EnglishLearningPlatformApp.DbMigrator")
    dotnet run
    dotnet run
...
$jobs += Start-Job -Name "DevCert" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/EnglishLearningPlatformApp.HttpApi.Host")
    $certificatePassPhrase = [Environment]::GetEnvironmentVariable("AuthServer__CertificatePassPhrase")
    if ([string]::IsNullOrWhiteSpace($certificatePassPhrase)) {
        throw "Set AuthServer__CertificatePassPhrase before generating the OpenIddict development certificate."
    }
    dotnet dev-certs https -v -ep openiddict.pfx -p $certificatePassPhrase
